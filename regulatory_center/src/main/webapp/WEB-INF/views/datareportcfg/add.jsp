...
<body>
	...
	<script>
	var dataSrc = 1;//由于查询的是视图表，默认新增配置的数据来源都是信贷
	function doSave(){
		debugger;
	    $.ajax({
	...
	function selectProject(){
		...
		var index = layer.open({
              ...
              yes: function(index){
                  ...
                  var trproj = "";
                  for(k =0 ; k<projectNum.length; k++){
                	  trproj +=projectNum[k].projId+"&"+projectNum[k].projName+","
                  }
                  if (trproj.length > 0) {
                	  trproj = trproj.substr(0,trproj.length - 1);
                  }
                  /* $("#org").val(trorg);
                  $("#orgname").val(trorg); */
                  //打印返回的值，看是否有我们想返回的值。
                  $("#project").val(trproj);
                  $("#projectname").val(trproj);
                  //最后关闭弹出层
                  layer.close(index);
              },
              cancel: function(){
		...
		var index = layer.open({
              ...
              yes: function(index){
                 ...
                 // $("#org").val(projectNum);
                  var trorg = "";
                  for(k =0 ; k<projectNum.length; k++){
                	 trorg +=projectNum[k].coOrgName+"-"+projectNum[k].coOrgCode+","
                  }
                  if (trorg.length > 0) {
                	  trorg = trorg.substr(0,trorg.length - 1);
                  }
                  $("#org").val(trorg);
                  $("#orgname").val(trorg);
                  //最后关闭弹出层
	...
	function selectProdure(){
		...
		var index = layer.open({
              ...
              yes: function(index){
                  ...
                  var trprod = "";
                  for(k =0 ; k<projectNum.length; k++){
                	  trprod +=projectNum[k].prodCode+"&"+projectNum[k].prodName+","
                  }
                  if (trprod.length > 0) {
                	  trprod = trprod.substr(0,trprod.length - 1);
                  }
                 /*  $("#org").val(trorg);
                  $("#orgname").val(trorg); */
