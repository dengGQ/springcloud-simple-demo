...
<body>
	<div class="container-fluid">
		<div>
			<div id="toolbar" class="btn-group group2">
				 <div class="btn-group">
			       <button class="btn btn-primary" id="editBtn">修改</button>
			    </div>
			</div>
			<table id="repay-table"></table>
		</div>
	</div>
	...
	$(function() {
		...
		initRepayTable();
		$('#repay-table').bootstrapTable('hideColumn', 'ACTUALID');
	});
	...
	function getRowsSeleced() {
		var rows = $('#repay-table').bootstrapTable('getSelections');
		return rows;
	};
	...
	function mathActualAmt(value,row){
		...
		console.log(intesPnlty);
		var result = 0;
		if(actualPrnpl == undefined && actualIntes == undefined && intesPnlty == undefined){
			return;
		}else{
			result = math.parser().eval(actualPrnpl + "+" + actualIntes+ "+" + intesPnlty)
		      if (isNaN(result)) { 
		        return; 
		      }
	...
	function initRepayTable() {
		var url = "${webRoot}/creditinfo/queryEditInfoByConNo";
		$('#repay-table').bootstrapTable(
		{
			method : 'GET',
			url : url,
			...
			onEditableSave: function (field, row, oldValue, $el) {
				  $table = $('#repay-table').bootstrapTable({});
