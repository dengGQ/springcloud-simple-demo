	...
	$(function() {
		...
		*/
		...
		$("#noSubmitBtn").click(function(){
			...
			var credType = getCretTypeSelections(selected);
		    $.ajax({
	            type: "post",
		...
		*/
		...
		$("#reBuildBtn").click(function (){
		 	layer.open({
				...
				moveOut: true,
				area : [ '995px', '150px' ],
				content : '${webRoot}/check/creditInspectionRebuild',
				cancel : function() {
				}
			}) 
		});
		...
		function getAccountSelections(rows) {
			 ...
			 * 获取选中的证件类型
			 */
			function getCretTypeSelections(rows) {
				var credType = "";
				for (var i = 0; i < rows.length; i++) {
					if ("" == credType) {
						credType = rows[i].credType;
					} else {
						credType = credType + "," + rows[i].credType;
					}
				}
				return credType;
			};
			...
			function getCredNoSelections(rows) {
				...
				for (var i = 0; i < rows.length; i++) {
					if ("" == credNo) {
						credNo = "'"+rows[i].credNo+"'";
					} else {
						credNo = credNo + "," +"'"+ rows[i].credNo+"'";
					}
				}
				return credNo;
			};
