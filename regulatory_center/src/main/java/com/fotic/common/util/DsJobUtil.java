...
//		System.out.println(retStr);\
	...
	public static String[] processCMDS(String jobName, String paramKey, String filePath, String paramFileName,
			String fileName) {
		return processCMDS(jobName, paramKey, filePath, paramFileName, fileName, false);
	}

	private static String[] processCMDS(String jobName, String paramKey, String filePath, String paramFileName,String fileName,
			boolean delLogFlag) {
		//第一步 -- 执行source命令
		String sourceCommand = new StringBuffer("source ").append(PropertiesUtil.get("ds.source")).toString();
		//第二步 -- 进入ds工具bin目录
		...
		//第三部 -- 调用dsjob
		StringBuffer jobInvokeCommand = new StringBuffer("./dsjob -run -mode NORMAL -param '").append(paramKey).append("'='").append(filePath)
				.append("' -param '").append(paramFileName).append("'='").append(fileName).append("'")
                .append(" -jobstatus ").append(PropertiesUtil.get("ds.user")).append(" ").append(jobName);
		//第四步 -- 读取重定向日志
        String findStatusCommand = null;
		//第五步 -- 删除日志文件
