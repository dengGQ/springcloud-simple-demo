	...
	var url = "${webRoot}/sms/querySmsSendDetailById";
		...
		function submitSms(){
			...
			var index = layer.confirm("是否确认提交本次短信发送内容信息？", {
				...
				sendSms(custCode,custName,phone,smsSendContent,coOrgCode,projId,prodCode,credType,credNo,iouNo);
			},function(){
				layer.close(index);
			});
		}
		
		function sendSms(custCode,custName,phone,smsSendContent,coOrgCode,projId,prodCode,credType,credNo,iouNo){
			$.ajax({
				type : 'post',
				url : '${webRoot}/sms/manulSendSms',
				data : { 'custCode' : custCode ,'custName':custName,'phone':phone,'smsSendContent':smsSendContent,'coOrgCode':coOrgCode,'projId':projId,'prodCode':prodCode,'credType':credType,'credNo':credNo,'iouNo':iouNo},
				beforeSend: function () {  
